...
export class NeoController {
  ...
  private async runGitCodexAction(command: GitActionCommand, action: DisplayAction): Promise<void> {
    ...
    try {
      ...
      if (!workingDirectory) throw new Error(this.gitSnapshot.error ?? "No local Git repository is available.");
      ...
      await action.showAlert();
      console.warn(`Local Codex Git action failed: ${command}`, error instanceof Error ? error.message : "unknown error");
    }
    await this.renderAll();
  ...
  private async readGitWorkspaces(snapshot: CodexSnapshot): Promise<GitWorkspaceContext[]> {
    ...
    if (snapshot.activeThreadId && snapshot.workspacePath && !observations.some((observation) => observation.threadId === snapshot.activeThreadId)) {
      observations.unshift({ threadId: snapshot.activeThreadId, status: snapshot.status === "offline" ? "idle" : snapshot.status, observedAt: snapshot.observedAt, workspacePath: snapshot.workspacePath });
    }
    ...
    return Promise.all([...unique.values()].map(async (observation) => ({ observation, snapshot: await this.gitAdapter.snapshot(observation.workspacePath) })));
  }
  ...
  private statusWorkspaceInfo() {
    const context = this.gitWorkspaces[this.statusWorkspaceIndex];
    return context ? { index: this.statusWorkspaceIndex, total: this.gitWorkspaces.length, snapshot: context.snapshot } : undefined;
  }
